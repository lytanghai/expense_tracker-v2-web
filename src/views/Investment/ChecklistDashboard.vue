        ...
        <main class="flex-1 p-6 overflow-y-auto">
            <div class="max-w-7xl mx-auto space-y-8">
                ...
                <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    <div v-for="plan in profitPlans.content" :key="plan.id"
                        ...
                        <p class="text-sm text-indigo-700">
                            Target: {{ formatPrice(plan.target, plan.currency) }}
                        </p>
                ...
                <div v-if="showCreateModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div class="bg-white rounded-3xl shadow-2xl w-full md:w-3/4 lg:w-1/2 p-6 relative z-10">
                        ...
                        <div class="grid gap-4 md:grid-cols-2">
                            ...
                            <div class="flex flex-col">
                                <label class="text-gray-700 font-semibold mb-1">Currency</label>
                        ...
                        <div class="flex justify-end mt-6">
                            <button @click="submitCreatePlan"
                                class="px-5 py-2 bg-green-500 text-white rounded-xl hover:bg-green-600 shadow">Create
                                Plan</button>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="bg-white rounded-3xl shadow-2xl w-full md:w-3/4 lg:w-1/2 p-6 relative z-10">
                        ...
                        <div class="flex justify-end mt-6">
                            <button @click="submitUpdate"
                                class="px-5 py-2 bg-yellow-500 text-white rounded-xl hover:bg-yellow-600 shadow">Update
                                Plan</button>
                        </div>
                    </div>
                </div>
                ...
                <div v-if="selectedPlan" class="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4">
                    ...
                    <!-- Modal Card -->
                    <div
                        class="bg-white rounded-3xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto p-4 sm:p-6 relative z-10">
                        ...
                        <table class="w-full table-auto text-left text-sm text-indigo-700 border border-gray-200">
                            ...
                            <tbody>
                                <tr v-for="detail in editableDetails" :key="detail.id">
                                    ...
                                    <td class="px-3 py-2 border">{{ formatPrice(detail.target, selectedPlan.currency) }}
                                    </td>
                        ...
                        <p class="mt-4 font-semibold">Grand Total:    {{ grandTotal }} {{ selectedPlan.currency }}
                        </p>
...
const targetTotal = computed(() => {
  const total = editableDetails.value.reduce((sum, d) => sum + d.target, 0);
  return Math.round(total * 100) / 100; // round to 2 decimals
});

const grandTotal = computed(() => {
  const total = editableDetails.value.reduce((sum, d) => sum + d.result, 0);
...
const openUpdateModal = (plan) => {
    ...
    updatePayload.currency = plan.currency || "USD";
